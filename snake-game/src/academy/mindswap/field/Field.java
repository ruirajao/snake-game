...
public final class Field {
    ...
    public static void drawSnake(Snake snake) {
        ...
        for (Position p : snake.getFullSnake()) {
            if (!p.equals(head)) {
                screen.putString(p.getXPosition(), p.getYPosition(), SNAKE_BODY_STRING, snakeColor, null);
            } else {
                screen.putString(p.getXPosition(), p.getYPosition(), SNAKE_HEAD_STRING, snakeColor, null);
            }
        }
        screen.refresh();
    }
    ...
    public static void clearTail(Snake snake) {
        ...
        screen.putString(tail.getXPosition(), tail.getYPosition(), " ", null, null);
    }
    ...
    public static void printString(int x, int y, String input){
        screen.putString(Field.getWidth()/2-4+x, Field.getHeight()/2+y,  input, Terminal.Color.YELLOW, null);
        screen.refresh(); //necessario para imprimir a mensagem do "game over"
    }
    ...
    public static void drawFruit(Fruit fruit) {
        screen.putString(fruit.getPosition().getXPosition(), fruit.getPosition().getYPosition(), FRUIT_STRING, Terminal.Color.MAGENTA, null);
    }
